...
with Diagram("Medallion Architecture - Contrôle de Gestion", show=False, direction="LR"):
    ...
    jenkins >> Edge(color="red") >> staging_bronze
    jenkins >> Edge(color="red") >> master_staging_bronze
    jenkins >> Edge(color="red") >> prod_bronze

    # Deployment Flow
    aws_cli >> Edge(color="orange") >> staging_bronze
    gitlab_ci >> Edge(color="orange") >> staging_bronze
    jenkins_pipeline >> Edge(color="orange") >> staging_bronze

    # Signature and Image
    #signature = Custom("Yahia Maamar","./signature.png")  # Replace with the path to your signature image , "./signature.png"
    logo = Custom("Logo", "./55 Brains.png")  # Replace with the path to your logo image

    # Position the signature and image
    #signature >> Edge(style="invis") >> logo
    Edge(style="invis") >> logo